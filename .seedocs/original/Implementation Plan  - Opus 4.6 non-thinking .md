...
# STATUS UPDATE: 2026-03-13 Implementation Audit

## Technology Stack Change
**Original Plan:** Python/FastAPI/SQLite/pytest  
**Actual Implementation:** TypeScript/Next.js/Bun/PostgreSQL/Prisma/vitest

## Overall Progress (228 tasks total)
| Status | Count | Percentage |
|--------|-------|------------|
| ✅ IMPLEMENTED | 75 | 33% |
| ⚠️ PARTIAL | 27 | 12% |
| ❌ NOT IMPLEMENTED | 123 | 54% |
| ⚙️ DIFFERENT APPROACH | 3 | 1% |

## Critical Gaps Blocking Progress
1. **Generation pipeline incomplete** - Storage methods (`_store*`) not fully implemented
2. **No generated knowledge** - Database empty due to API rate limits during testing
3. **Missing tests** - Only 36 tests, most functionality untested
4. **No Docker setup** - Cannot deploy easily
5. **Vitality system** - UI exists but no real data computation

## What Works Now
- ✅ Complete frontend (Next.js 14, App Router, React Query, Zustand)
- ✅ 15+ API endpoints serving knowledge
- ✅ LLM client with retry logic and rate limiting
- ✅ Response parser with 5 recovery strategies
- ✅ Validation engine with Python code execution
- ✅ Rich UI components (BundlePanel, AlgorithmView, CapabilityTree)
- ✅ 36 passing tests

## Next Priority Tasks
1. Complete generation executor storage methods (Tasks 77-83)
2. Run actual generation pipeline for JSON target (Tasks 89-94)
3. Add comprehensive API tests (Tasks 127-131)
4. Implement Docker deployment (Tasks 208-212)
5. Complete documentation (Tasks 214-217)
...
|---|------|------|------------|--------|
| ✅ 1 | Create monorepo: `magb-poc/backend/`, `magb-poc/frontend/`, `magb-poc/docs/`, `magb-poc/scripts/` | 20m | — | ⚠️ PARTIAL - TypeScript/Next.js instead of Python |
| ✅ 2 | Create `backend/pyproject.toml` with all dependencies (fastapi, uvicorn, pydantic, aiohttp, pyyaml, rich, tenacity, click) | 15m | 1 | ❌ NOT IMPLEMENTED - Using TypeScript/Bun |
| ✅ 3 | Create backend Python package structure: `src/magb/` with all subpackages (`core/`, `generation/`, `validation/`, `observability/`, `api/`, `output/`) — all `__init__.py` files, empty modules | 30m | 2 | ❌ NOT IMPLEMENTED - Using TypeScript src/engine/ |
| ✅ 4 | `pip install -e ".[dev]"` — verify import: `from magb.core.schema import *` | 10m | 3 | ❌ NOT IMPLEMENTED - Using Bun/npm |
| ✅ 5 | Create frontend with `pnpm create next-app` (App Router, TypeScript, Tailwind, src/) | 10m | 1 | ✅ IMPLEMENTED - Next.js 14 with App Router |
| ✅ 6 | Install frontend deps: `@tanstack/react-query`, `zustand`, `@radix-ui/react-tabs`, `@radix-ui/react-dialog`, `@radix-ui/react-tooltip` | 10m | 5 | ✅ IMPLEMENTED - All deps in package.json |
| ✅ 7 | Create frontend directory structure: `src/app/` routes, `src/components/`, `src/hooks/`, `src/lib/`, `src/stores/`, `src/types/` | 20m | 6 | ✅ IMPLEMENTED |
| ✅ 8 | Verify `pnpm dev` shows Next.js page | 5m | 7 | ✅ IMPLEMENTED |

**Afternoon: Configuration**

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 9 | Create `backend/config.yaml` with all POC settings (llm, generation, storage, api, observability sections) | 20m | 3 | ⚠️ PARTIAL - config.ts exists, no yaml |
| ✅ 10 | Create `.env.example` with `ZEN_API_KEY=` | 5m | 1 | ⚠️ PARTIAL - ZAI_API_KEY used instead |
| ✅ 11 | Implement `magb/config.py`: Pydantic dataclasses for LLMConfig, GenerationConfig, StorageConfig, APIConfig, ObservabilityConfig | 45m | 4 | ⚠️ PARTIAL - config.ts exists with similar structure |
| ✅ 12 | Implement `load_config(path)` function with YAML loading and env var override | 30m | 11 | ⚠️ PARTIAL - config.ts loads from env |
| ✅ 13 | Write `test_config.py` — config loads, defaults apply, env vars override | 20m | 12 | ❌ NOT IMPLEMENTED - No tests |
| ✅ 14 | Create `Makefile` with targets: install, dev-backend, dev-frontend, test, lint, generate | 20m | 4, 8 | ❌ NOT IMPLEMENTED - Using npm scripts |
...
| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 15 | Implement all enums in `core/schema.py`: TargetKind (21 values), ConceptDomain (18), RelationshipType (18+), Complexity (5), Confidence (5), DecayModel (5 with HALF_LIFE_DAYS dict) | 60m | 4 | ⚠️ PARTIAL - Enums in Prisma schema, not Python |
| ✅ 16 | Implement `KnowledgeNode` class: constructor, `_compute_hash()` (SHA-256), `to_dict()`, `generate_id()` static method | 45m | 15 | ❌ NOT IMPLEMENTED - Using Prisma/PostgreSQL |
| ✅ 17 | Implement `KnowledgeEdge` class: constructor, `_compute_id()`, `to_dict()` | 20m | 15 | ❌ NOT IMPLEMENTED - Using Prisma relations |
| ✅ 18 | Write `test_schema.py`: ID generation determinism, hash change detection, hash stability, to_dict roundtrip, edge ID determinism | 30m | 16, 17 | ❌ NOT IMPLEMENTED - No tests |

**Afternoon: SQLite store**

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 19 | Implement `UniversalKnowledgeStore.__init__()` and `_init_schema()`: all tables (nodes, edges, generation_runs, validations), all indices, WAL mode, foreign keys | 60m | 16, 17 | ✅ IMPLEMENTED - Using Prisma/PostgreSQL |
| ✅ 20 | Implement `upsert_node()`: INSERT OR REPLACE, content_hash change detection, return bool | 30m | 19 | ⚠️ PARTIAL - store/index.ts has getStatistics |
| ✅ 21 | Implement `get_node()`, `get_nodes_by_type()`, `get_nodes_by_tag()` | 30m | 19 | ⚠️ PARTIAL - Partial implementation |
| ✅ 22 | Implement `add_edge()` | 15m | 19 | ⚠️ PARTIAL - Relations via Prisma |
| ✅ 23 | Implement `_row_to_node_dict()` helper | 15m | 19 | ❌ NOT IMPLEMENTED |
| ✅ 24 | Write `conftest.py` fixtures: `tmp_db`, `store`, `sample_nodes`, `sample_edges` | 30m | 19 | ❌ NOT IMPLEMENTED - No tests |
| ✅ 25 | Write `test_store.py` part 1: upsert creates, upsert detects no-change, upsert detects change, get_node found, get_node missing, get_nodes_by_type | 30m | 20-24 | ❌ NOT IMPLEMENTED - No tests |
...
| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 26 | Implement `get_neighbors(node_id, relationship, direction)` with outgoing/incoming/both support | 45m | 22 | ✅ IMPLEMENTED - store/index.ts has getNeighbors |
| ✅ 27 | Implement `get_subgraph(root_id, depth, relationship_filter)` — BFS traversal | 45m | 26 | ❌ NOT IMPLEMENTED |
| ✅ 28 | Implement `find_path(from_id, to_id, max_depth)` — BFS pathfinding | 30m | 26 | ❌ NOT IMPLEMENTED |
| ✅ 29 | Implement FTS5 full-text search: create virtual table in schema, implement `search_text(query, limit)` | 30m | 19 | ⚠️ PARTIAL - PostgreSQL full-text search via Prisma |
| ✅ 30 | Implement `get_statistics()` | 15m | 19 | ✅ IMPLEMENTED - store/index.ts has getStatistics |
| ✅ 31 | Write `test_store.py` part 2: neighbors outgoing, neighbors incoming, neighbors filtered, subgraph depth 1/2, find_path direct/2-hop, search_text by name/content, statistics | 45m | 26-30 | ❌ NOT IMPLEMENTED - No tests |

**Afternoon: Zen AI SDK integration**

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 32 | Research Zen AI SDK: exact import path, call pattern, response format, supported models, token counting, streaming support. Document findings in `docs/ZEN_AI_SDK.md` | 60m | 10 | ⚠️ PARTIAL - docs/poc/zai-setup.md exists |
| ✅ 33 | Implement `ZenAIClient.__init__(config)`: initialize SDK with API key, set default model | 30m | 32, 12 | ✅ IMPLEMENTED - ZaiClient.ts |
| ✅ 34 | Implement `ZenAIClient.call(prompt, system_prompt, temperature, max_tokens, response_format)` — wrap SDK call in standard interface, return `{content, tokens, cost}` | 45m | 33 | ⚠️ PARTIAL - generateText and generateJson exist |
| ✅ 35 | **FIRST REAL API CALL:** Test with simple prompt: `"List the 5 primary data types in JSON. Respond as JSON: {types: [...]}"` — verify response, record token usage | 15m | 34 | ❌ NOT VERIFIED - Rate limits prevented |
...
| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 36 | Implement rate limiter in ZenAIClient: token bucket, `async acquire(estimated_tokens)`, RPM + TPM tracking | 45m | 34 | ❌ NOT IMPLEMENTED - Uses OpenAI retry |
| ✅ 37 | Implement cost tracker: total input/output tokens, total USD, `budget_remaining()`, raise `BudgetExhausted` on limit | 30m | 34 | ❌ NOT IMPLEMENTED |
| ✅ 38 | Implement retry logic with tenacity: exponential backoff (2s/4s/8s), max 3 retries, retry on 429/500/502/503, no retry on 401/400 | 30m | 34 | ⚠️ PARTIAL - OpenAI maxRetries: 5 |
| ✅ 39 | Implement `TokenEstimator`: `estimate_tokens(text)` (word count × 1.3), `estimate_cost(input, output, model)` | 20m | 4 | ❌ NOT IMPLEMENTED |

**Afternoon: Response parser**

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 40 | Implement `ParseError` exception with raw_response and partial_result | 10m | 4 | ✅ IMPLEMENTED - ResponseParser.ts |
| ✅ 41 | Implement `ResponseParser.parse_json()` — main entry, tries 5 strategies | 15m | 40 | ✅ IMPLEMENTED - ResponseParser.ts |
| ✅ 42 | Implement Strategy 1: `_try_direct_parse()` | 5m | 41 | ✅ IMPLEMENTED |
| ✅ 43 | Implement Strategy 2: `_try_code_fence_extraction()` — regex for ```json ``` | 15m | 41 | ✅ IMPLEMENTED |
| ✅ 44 | Implement Strategy 3: `_try_boundary_extraction()` — find { } pairs with depth/string tracking | 30m | 41 | ⚠️ PARTIAL - Partial |
| ✅ 45 | Implement Strategy 4: `_try_cleaned_parse()` — remove fences, trailing commas, comments, single quotes, prefix/suffix text | 30m | 41 | ⚠️ PARTIAL - Partial |
| ✅ 46 | Implement Strategy 5: `_try_truncation_repair()` — count open brackets, close strings, remove partials, append closers, progressive retry | 45m | 41 | ⚠️ PARTIAL - Partial |
| ✅ 47 | Implement `_validate()` — optional Pydantic schema validation, warn on fail but return data | 15m | 41 | ⚠️ PARTIAL - Zod validation |
...
| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 48 | Create 10 test fixture files in `tests/fixtures/`: clean_json, code_fenced, trailing_comma, with_comments, truncated_simple, truncated_nested, mixed_text_json, single_quotes, empty_response, no_json | 30m | 41 | ❌ NOT IMPLEMENTED - No test fixtures |
| ✅ 49 | Write `test_parser.py`: one test per fixture, one per strategy, test ParseError on empty/no-json, test Pydantic validation pass/warn | 60m | 48, 42-47 | ❌ NOT IMPLEMENTED - No tests |

**Afternoon: Real prompt testing — THE CRITICAL MOMENT**

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 50 | Implement `SYSTEM_PROMPT` constant in `prompts.py` | 10m | 4 | ✅ IMPLEMENTED - prompts.ts exists |
| ✅ 51 | Implement `PromptTemplates.discover_capabilities(target, target_type)` for filetype | 30m | 50 | ✅ IMPLEMENTED - prompts.ts |
| ✅ 52 | **CRITICAL TEST:** Send discover_capabilities("json", "filetype") through Zen AI. Save raw response as fixture. Parse it. Verify JSON capabilities found (objects, arrays, strings, numbers, booleans, null). Count total. Record tokens and cost | 30m | 51, 34, 41 | ❌ NOT COMPLETED - Rate limited |
| ✅ 53 | Implement `PromptTemplates.extract_structural_template(target, capability)` | 30m | 50 | ✅ IMPLEMENTED - prompts.ts |
| ✅ 54 | **CRITICAL TEST:** Send extract_structural_template("json", {"id": "nested_object", "name": "Nested Object"}) through Zen AI. Save response. Verify template, variables, assembly code present | 30m | 53, 34 | ❌ NOT COMPLETED - Rate limited |
| ✅ 55 | Implement `PromptTemplates.extract_algorithm(target, capability)` | 30m | 50 | ✅ IMPLEMENTED - prompts.ts |
| ✅ 56 | **CRITICAL TEST:** Send extract_algorithm prompt through Zen AI. Verify Python implementation is syntactically valid. Verify test vectors present | 30m | 55, 34 | ❌ NOT COMPLETED - Rate limited |
| ✅ 57 | Document actual token usage per prompt type. Update cost model. Verify budget is sufficient for full JSON generation | 15m | 52, 54, 56 | ❌ NOT COMPLETED |
...
| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 58 | Implement `extract_coordinate_system(target)` prompt | 20m | 50 | ✅ IMPLEMENTED |
| ✅ 59 | Implement `extract_minimal_valid_file(target)` prompt | 20m | 50 | ✅ IMPLEMENTED |
| ✅ 60 | Implement `cross_reference_validate(target, section, items)` prompt | 20m | 50 | ⚠️ PARTIAL - Some prompts exist |
| ✅ 61 | Implement `extract_composition_rules(target, capabilities)` prompt | 20m | 50 | ⚠️ PARTIAL |
| ✅ 62 | Implement `generate_blueprint(target, app_type, capabilities)` prompt | 20m | 50 | ⚠️ PARTIAL |
| ✅ 63 | Test each new prompt with a single real API call. Save responses as fixtures. Fix any that produce poor output | 90m | 58-62 | ❌ NOT COMPLETED - Rate limited |

### Day 7: Generation Planner

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 64 | Implement `GenerationTask` dataclass: id, target, section, subsection, phase (SKELETON/EXPANSION/CROSS_REF/GAP_FILL/SYNTHESIS), prompt, status (PENDING/IN_PROGRESS/COMPLETED/FAILED), result, token_cost, retries, depends_on | 30m | 4 | ✅ IMPLEMENTED - planner.ts |
| ✅ 65 | Implement `GenerationPlanner.generate_layer1_tasks(target, target_type)`: capability discovery + coordinate system + minimal file = 3 tasks | 30m | 64 | ✅ IMPLEMENTED |
| ✅ 66 | Implement `GenerationPlanner.generate_layer2_tasks(target, capabilities)`: one template + one algorithm per capability, plus cross-ref tasks | 45m | 64, 65 | ✅ IMPLEMENTED |
| ✅ 67 | Implement `GenerationPlanner.generate_layer3_tasks(target, capabilities)`: composition rules batches + blueprints | 30m | 64, 66 | ✅ IMPLEMENTED |
| ✅ 68 | Implement `_extract_capabilities_from_discovery(result)`: flatten hierarchy, preserve categories | 30m | 64 | ⚠️ PARTIAL |
| ✅ 69 | Implement `BLUEPRINT_TEMPLATES` dict: default blueprint types per format kind | 15m | 67 | ❌ NOT IMPLEMENTED |
| ✅ 70 | Write `test_planner.py`: layer 1 produces 3 tasks, layer 2 produces correct count, layer 3 produces composition + blueprint tasks, dependencies are correct | 30m | 65-68 | ❌ NOT IMPLEMENTED - No tests |

### Day 8: Checkpoint Manager

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 71 | Add `generation_checkpoints` table to store schema | 15m | 19 | ✅ IMPLEMENTED - CheckpointManager.ts uses JSON files |
| ✅ 72 | Implement `CheckpointManager`: `get_completed_tasks()`, `mark_started()`, `mark_completed()`, `mark_failed()`, `get_run_summary()` | 45m | 71 | ✅ IMPLEMENTED |
| ✅ 73 | Write `test_checkpoint.py`: start/complete flow, get_completed returns only completed, failed records error, summary aggregates | 30m | 72 | ❌ NOT IMPLEMENTED - No tests |

### Days 9-10: Generation Executor Core

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 74 | Implement `GenerationExecutor.__init__(store, llm_client, planner, config)`: create semaphore, checkpoint manager | 20m | 72, 34, 65 | ✅ IMPLEMENTED - executor.ts |
| ✅ 75 | Implement `_execute_single_task(task)`: check budget, check deps, mark started, call LLM, parse response, store result, mark completed. On failure: retry or mark failed | 90m | 74 | ⚠️ PARTIAL - executor.ts exists |
| ✅ 76 | Implement `_execute_batch(tasks)`: filter completed (from checkpoint), asyncio.gather with semaphore | 45m | 75 | ⚠️ PARTIAL |
| ✅ 77 | Implement `_store_discovery_results(target, data)`: create target node, create capability nodes, create "contains" edges, return capability list | 60m | 76 | ❌ NOT IMPLEMENTED |
| ✅ 78 | Implement `_store_template_result(target, cap_id, data)`: create structure node, create "template_for" edge | 30m | 77 | ❌ NOT IMPLEMENTED |
| ✅ 79 | Implement `_store_algorithm_result(target, cap_id, data)`: create algorithm node, create "uses_algorithm" edge | 30m | 77 | ❌ NOT IMPLEMENTED |
| ✅ 80 | Implement `_store_coordinate_system(target, data)`: create node with "coordinate" tag, edge from target | 20m | 77 | ❌ NOT IMPLEMENTED |
| ✅ 81 | Implement `_store_minimal_file(target, data)`: create structure node with "minimal" tag | 20m | 77 | ❌ NOT IMPLEMENTED |
| ✅ 82 | Implement `_store_blueprint(target, data)`: create blueprint node, "builds_with" edges | 20m | 77 | ❌ NOT IMPLEMENTED |
| ✅ 83 | Implement `_store_composition_rules(target, data)`: create concept node with "composition" tag | 20m | 77 | ❌ NOT IMPLEMENTED |

### Days 11-12: Executor Pipeline Orchestration

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 84 | Implement `execute_full_generation(target, version, target_type, resume, progress_callback)`: Layer 1 → parse caps → Layer 2 → Layer 3 → gap detection → gap fill → statistics | 120m | 76-83 | ⚠️ PARTIAL - executor.ts exists |
| ✅ 85 | Implement `_run_gap_detection(target, capabilities)`: create cross-ref tasks, parse results, return gap-fill tasks | 45m | 84 | ❌ NOT IMPLEMENTED |
| ✅ 86 | Implement progress callback: call with percentage at each phase transition | 15m | 84 | ❌ NOT IMPLEMENTED |
| ✅ 87 | Write `test_executor.py` with mocked LLM client: full pipeline completes, resume skips completed, budget enforcement stops, retry logic, dependency ordering | 90m | 84 | ❌ NOT IMPLEMENTED - No tests |

### Days 13-15: First JSON Generation

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 88 | Create `scripts/generate_target.py` CLI tool: accept target name, version, type. Progress bar with Rich. Cost display | 45m | 84 | ✅ IMPLEMENTED - scripts/generate_target.ts |
| ✅ 89 | **RUN GENERATION:** Execute full pipeline for JSON (RFC 8259, filetype). Monitor progress. Record all output | 120m | 88 | ❌ NOT COMPLETED - Rate limited |
| ✅ 90 | Review Layer 1 results: capabilities discovered (expect 30+), check for key features (objects, arrays, strings, numbers, booleans, null, nesting, escape sequences, unicode, encoding, whitespace, comments in JSON5, schema validation) | 30m | 89 | ❌ NOT COMPLETED |
| ✅ 91 | Review Layer 2 results: structural templates valid, algorithm implementations parse as Python, test vectors present | 45m | 89 | ❌ NOT COMPLETED |
| ✅ 92 | Review Layer 3 results: blueprints coherent, composition rules present | 30m | 89 | ❌ NOT COMPLETED |
| ✅ 93 | Fix prompt issues: iterate on any prompts that produced poor output, re-run failed tasks | 120m | 90-92 | ❌ NOT COMPLETED |
| ✅ 94 | Re-run with fixes if needed | 60m | 93 | ❌ NOT COMPLETED |

### Day 16: Post-Generation Verification

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 95 | Record final generation statistics: total nodes, edges, API calls, tokens, cost, wall-clock time, success rate | 30m | 94 | ❌ NOT COMPLETED |
| ✅ 96 | Query database: count nodes by type, count edges by relationship, verify no orphan nodes, verify no dangling edges | 30m | 94 | ❌ NOT COMPLETED |
| ✅ 97 | Extract 3 Python implementations from generated algorithms. Compile each (syntax check). Record results | 30m | 94 | ❌ NOT COMPLETED |
| ✅ 98 | Extract minimal file generation code. Run it. Verify output is valid JSON | 30m | 94 | ❌ NOT COMPLETED |
...
| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 99 | Implement `api/app.py`: FastAPI factory, lifespan (init store + query engine), CORS from config, include routes | 45m | 19, 12 | ⚠️ PARTIAL - Replaced by Next.js API Routes |
| ✅ 100 | Implement `api/deps.py`: `get_store()`, `get_query_engine()`, `verify_api_key()` (accept any non-empty key) | 30m | 99 | ⚠️ PARTIAL - lib/store.ts exists |
| ✅ 101 | Implement `api/middleware.py`: request timing, request ID, request logging | 30m | 99 | ❌ NOT IMPLEMENTED |
| ✅ 102 | Implement `APIResponse` Pydantic model | 10m | 99 | ❌ NOT IMPLEMENTED - Ad-hoc Next.js responses |
| ✅ 103 | Implement `GET /v1/health`: status, uptime, node counts. No auth required | 15m | 99 | ✅ IMPLEMENTED - /api/v1/health |
| ✅ 104 | Verify: `uvicorn magb.api.app:app --reload` starts, /docs loads, /v1/health returns 200 | 10m | 103 | ✅ IMPLEMENTED - next dev |

### Day 18: Explore Endpoints

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 105 | Implement `GET /v1/targets`: kind/search/sort_by/limit/offset params, capability counts, vitality scores | 60m | 104 | ✅ IMPLEMENTED - /api/v1/targets |
| ✅ 106 | Implement `GET /v1/targets/{target_id}`: capability tree by category, has_template/has_algorithm per capability, vitality info, 404 with helpful message | 60m | 104 | ✅ IMPLEMENTED - /api/v1/targets/[targetId] |
| ✅ 107 | Implement `GET /v1/targets/{target_id}/capabilities`: category/complexity/has_algorithm/search filters | 45m | 106 | ✅ IMPLEMENTED - /api/v1/targets/[targetId]/capabilities |
| ✅ 108 | Implement `GET /v1/graph/neighbors/{node_id}`: relationship/direction/depth params, subgraph at depth>1 | 45m | 104 | ✅ IMPLEMENTED - /api/v1/graph/neighbors/[nodeId] |
| ✅ 109 | Implement `GET /v1/concepts`: domain/search/limit filters | 30m | 104 | ❌ NOT IMPLEMENTED |

### Day 19: Retrieve Endpoints

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 110 | Implement `GET /v1/capabilities/{id}/bundle`: assemble capability + templates + algorithms + coordinates + composition + prerequisites. Language filter. response_format variants | 90m | 104 | ✅ IMPLEMENTED - /api/v1/capabilities/[capabilityId]/bundle |
| ✅ 111 | Implement `_extract_code_only()`, `_extract_summary()`, `_format_for_ai_context()` helper functions | 30m | 110 | ❌ NOT IMPLEMENTED |
| ✅ 112 | Implement `GET /v1/algorithms/{id}`: full algorithm with preferred implementation, used_by list | 45m | 104 | ✅ IMPLEMENTED - /api/v1/algorithms/[algorithmId] |
| ✅ 113 | Implement `GET /v1/structures/{id}`: template with served capabilities | 30m | 104 | ✅ IMPLEMENTED - /api/v1/structures/[structureId] |
| ✅ 114 | Implement `GET /v1/targets/{id}/coordinate-system` | 15m | 104 | ✅ IMPLEMENTED - /api/v1/targets/[targetId]/coordinate-system |
| ✅ 115 | Implement `GET /v1/targets/{id}/minimal-file` | 15m | 104 | ✅ IMPLEMENTED - /api/v1/targets/[targetId]/minimal-file |

### Day 20: Synthesize Endpoints

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 116 | Implement `KnowledgeQuery` class: `assemble_context_for_task()` — search, score (target proximity + node type + confidence), include coordinates, assemble within token budget, format per type | 90m | 29 | ❌ NOT IMPLEMENTED - Inline logic used |
| ✅ 117 | Implement `KnowledgeQuery.compare_targets()` — shared algorithms, shared concepts, unique to each, similarity score | 30m | 116 | ❌ NOT IMPLEMENTED |
| ✅ 118 | Implement `POST /v1/assemble`: accept target/task/language/tests/tokens, use KnowledgeQuery, categorize by type, return bundle | 45m | 116 | ✅ IMPLEMENTED - /api/v1/assemble |
| ✅ 119 | Implement `POST /v1/blueprint`: search for matching blueprints, fuzzy match on description, return with dependencies | 30m | 104 | ❌ NOT IMPLEMENTED |
| ✅ 120 | Implement `POST /v1/diagnose`: heuristic cause detection (coordinate/structural/color/relationship keywords), return causes + constraints + coordinate system | 30m | 104 | ❌ NOT IMPLEMENTED |
| ✅ 121 | Implement `POST /v1/convert` and `POST /v1/compare` | 30m | 117 | ❌ NOT IMPLEMENTED |

### Day 21: Search + AI Context Endpoints

| # | Task | Est. | Depends On | Status |
|---|------|------|------------|--------|
| ✅ 122 | Implement `POST /v1/search`: query/node_types/targets/limit, full-text search with filters, return with snippets | 30m | 29 | ✅ IMPLEMENTED - /api/v1/search |
| ✅ 123 | Implement `GET /v1/search/algorithms`: domain/has_implementation filters, return with availability info | 30m | 122 | ❌ NOT IMPLEMENTED |
| ✅ 124 | Implement `POST /v1/ai/context`: return plain text (Content-Type: text/plain), wrap in knowledge header/footer | 30m | 116 | ✅ IMPLEMENTED - /api/v1/ai/context |
| ✅ 125 | Implement `GET /v1/ai/system-prompt/{target_id}`: build system prompt from target knowledge, include coordinates + key templates | 30m | 104 | ❌ NOT IMPLEMENTED |
| ✅ 126 | Implement meta endpoints: `GET /v1/meta/node-types`, `/relationship-types`, `/statistics` | 20m | 104 | ⚠️ PARTIAL - /api/v1/meta/statistics exists |
